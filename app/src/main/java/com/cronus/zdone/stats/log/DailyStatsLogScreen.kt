...
class DailyStatsLogScreen @Inject constructor(
    private val taskEventsDao: TaskEventsDao,
    private val toaster: Toaster
) : CoroutineScreen<DailyStatsLogView>() {
    ...
    override fun createView(context: Context): DailyStatsLogView =
        DailyStatsLogView(context)

    override fun getTitle(context: Context?) = "log"
    ...
    private fun delete(taskEvent: TaskEvent) {
        safeLaunch {
            withContext(Dispatchers.IO) {
...
private fun DailyStatsLogScreen.TaskEventUpdate.getNewTaskEvent(): TaskEvent {
...
private fun DailyStatsLogScreen.TaskEventUpdate.valid(): Boolean {
    return newName.isNotEmpty() &&
            isValidNewDuration() &&
            isValidNewEndTime()
}

private fun DailyStatsLogScreen.TaskEventUpdate.isValidNewEndTime() =
    newEndTime.matches(Regex("\\d\\d:\\d\\d"))

private fun DailyStatsLogScreen.TaskEventUpdate.isValidNewDuration() =
